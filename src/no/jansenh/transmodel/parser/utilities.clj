;;-----------------------------------------------------------------------------
;; File: src/no/jansenh/transmodel/parser/utilities.clj
...
(ns no.jansenh.transmodel.parser.utilities
  (:import [java.time Instant ZonedDateTime LocalDateTime OffsetDateTime ZoneOffset]
           [java.time.format DateTimeFormatter DateTimeParseException]))

;; NeETx parser utilities
;;
;; The namepace handle XML acrobatics:
;;   - attributes on a tag
;;   - all attributes on a tag
;;   - tag element
;;   - tag text
;;
;; The namespace handle DateTime with not-yet-tested elegance:
...
(defn parse-datetime
  ...
  [datetime-str]
  (when (and datetime-str (not (empty? datetime-str)))
    (try
...
(defn tag-attr-datetime
  "Get datetime attribute from a tag and parse it to Instant.
